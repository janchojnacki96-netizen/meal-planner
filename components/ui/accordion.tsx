...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-3 text-left text-sm font-semibold text-slate-900 transition hover:text-slate-700 [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
